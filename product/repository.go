...
type Repository interface {
	GetProducts(param *getProductsRequest) ([]*Product, error)
...
func (repo *repository) GetProducts(param *getProductsRequest) ([]*Product, error) {
	...
	for results.Next() {
		product := &Product{}
		results.Scan(&product.Id, &product.ProductCode, &product.ProductName, &product.Description, &product.StandardCost,
...
func (repo *repository) DeleteProduct(productId int) (int64, error) {
	sql := `DELETE FROM products WHERE id=?`

	result, err := repo.db.Exec(sql, productId)
	helper.Catch(err)
